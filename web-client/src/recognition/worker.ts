...
export function validateModelMetadata(
  ...
  requireContract(model.path === "digits-crnn.onnx", "unexpected model path");
  ...
  requireContract(
    Array.isArray(input.shape) && arraysEqual(input.shape, MODEL_INPUT_SHAPE),
  ...
  requireContract(
    typeof confidence.confidence_threshold === "number" &&
      confidence.confidence_threshold ===
        marginConfidence(confidence.raw_threshold as number),
...
export function verifyModelBytes(
  ...
  if (data.byteLength !== model.bytes) {
    throw new Error(
      `model byte length ${data.byteLength} does not match metadata ${model.bytes}`,
    );
  }
...
function validateSession(
  ...
  requireContract(
    input.isTensor && arraysEqual(input.shape, MODEL_INPUT_SHAPE),
  ...
  requireContract(
    output.isTensor && arraysEqual(output.shape, MODEL_OUTPUT_SHAPE),
...
export class OrtRecognitionBackend implements RecognitionWorkerBackend {
  ...
  async initialize(
    ...
    if (!response.ok) {
      throw new Error(
        `model metadata request failed with HTTP ${response.status}`,
      );
    }
    ...
    if (!modelResponse.ok) {
      throw new Error(
        `recognition model request failed with HTTP ${modelResponse.status}`,
      );
    }
...
if (
  ...
  workerScope.addEventListener("message", (event) => handleMessage(event.data));
}
