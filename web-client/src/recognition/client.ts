...
function defaultAssetBaseUrl(): string {
  ...
  return new URL(import.meta.env.BASE_URL, location).href;
}
...
export class RecognitionClient {
  ...
  recognize(input: RecognitionInput, revision: number): Promise<Recognition> {
    ...
    if (revision !== this.currentRevision) {
      return Promise.reject(
        runtimeError(
          "stale_revision",
          `recognition revision ${revision} is not current revision ${this.currentRevision}`,
          "protocol",
          false,
        ),
      );
  ...
  private handleTimeout(requestId: number): void {
    ...
    this.stopWorker(
      runtimeError(
        "inference_timeout",
        `recognition timed out after ${this.timeoutMs} ms; worker restarted`,
        "inference",
      ),
    );
