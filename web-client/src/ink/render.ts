...
export function watchDevicePixelRatio(
  ...
  const arm = () => {
    mediaQuery = source.matchMedia(
      `(resolution: ${normalizedDevicePixelRatio(source.devicePixelRatio)}dppx)`,
    );
...
function paintStroke(
  ...
  if (points.length === 1) {
    paintDot(context, points[0], widths[0]);
    return;
  }
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    const end =
      index === points.length - 1
        ? control
        : midpoint(control, points[index + 1]);

    context.beginPath();
    context.moveTo(start.x, start.y);
    context.quadraticCurveTo(control.x, control.y, end.x, end.y);
    context.lineWidth = (widths[index - 1] + widths[index]) / 2;
    context.stroke();
    start = { ...control, ...end };
  }
}
