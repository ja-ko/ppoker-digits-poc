...
describe("vote input reducer", () => {
  ...
  it("guards stale effects and prevents pointer input against committed state", () => {
    ...
    expect(reduce(committed, { type: "POINTER_ACCEPTED", revision: 2 })).toBe(
      committed,
    );
  });
});
...
describe("recognition disposition", () => {
  ...
  it.each([
    ...
    (text, confidence, deck, expected) => {
      expect(
        classifyRecognition({ text, confidence }, 0.9, deck as number[]),
      ).toEqual(expected);
    },
  );
});
