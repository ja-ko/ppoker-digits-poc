...
- [x] Use a fullscreen voting view with input restricted to the marked writing
      zone; never retain or render samples outside that zone.
...
- [x] Low-confidence input shakes and disappears after the inactivity grace
      period.
- [x] Every rejected input shakes and then disappears under normal motion.
...
- [x] Proper prefixes receive a longer wait and then shake as incomplete input.
...
| Proper prefix only, then reject as incomplete |             about 1,100 ms |
...
- [x] A committed result is displayed as a large typeset number centered in the
      writing zone.
- [x] The original trace translates toward the center while fading and
      contracting into that number.
...
- [x] Every rejection shakes before fading unless reduced motion is enabled.
...
- [x] Keep the displayed deck informational and prevent taps on it from falling
      through to the handwriting surface.
...
- [x] Delay a proper-prefix-only result until its extended deadline, then reject
      it with the same shake choreography.
...
- [x] On commit, translate and tighten the trace toward the writing-zone center.
- [x] Crossfade the trace into a clean typeset number.
- [x] Add a restrained landing scale/easing to the typeset result.
- [x] On every rejection, shake the trace horizontally and then fade it.
...
- [x] Keep a compact last-decision helper visible after commit or rejection with
      confidence, the rejected candidate, and deck-membership status.
