...
export function rasterizeInk(
  ...
  for (const stroke of strokes) {
    ...
    if (points.length === 1) {
      paintCapsule(sampleMasks, points[0], points[0], config);
      continue;
    }
    for (let index = 1; index < points.length; index += 1) {
      paintCapsule(sampleMasks, points[index - 1], points[index], config);
    }
  }
  ...
  for (let index = 0; index < sampleMasks.length; index += 1) {
    data[index] = countSetBits(sampleMasks[index]) / samplesPerPixel;
  }
...
export function rgbaToNchw(
  ...
  for (let index = 0; index < data.length; index += 1) {
    ...
    data[index] = (luminance / 255) * (rgba[rgbaIndex + 3] / 255);
  }
  return data;
}
